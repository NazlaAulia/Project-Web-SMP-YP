        ...
        <div class="register-left" data-aos="fade-right">
            <div class="brand-row">
                ...
                <div class="top-login-text">
                    Sudah pernah daftar? <a href="login.php">Masuk Sekarang!</a>
                </div>
            </div>
            ...
            <div id="waReminder" class="wa-reminder-box" style="display:none;">
                <strong>Silakan lanjut konfirmasi ke admin.</strong>
                <a href="#" id="waReminderLink" target="_blank">Chat Admin via WhatsApp</a>
            </div>
...
<div class="modal-overlay" id="successModal">
    <div class="modal-box">
        ...
        <div class="modal-actions">
            ...
            <a href="#" id="waAdminBtn" class="btn-wa" target="_blank" onclick="closeModal()">
                Konfirmasi ke Admin
            </a>
        </div>
    </div>
...
const firebaseConfig = {
  ...
  databaseURL: "https://smpyp17-default-rtdb.asia-southeast1.firebasedatabase.app",
  projectId: "smpyp17",
...
form.addEventListener("submit", async function(e) {
    ...
    const namaLengkap = form.nama_lengkap.value.trim();
    const nisn = form.nisn.value.trim();
    ...
    const asalSekolah = form.asal_sekolah.value.trim();
    const noHp = form.no_hp.value.trim();
    const email = form.email.value.trim();
    const pendapatanOrtu = form.pendapatan_ortu.value.trim();

    if (!namaLengkap || !nisn || !jenisKelamin || !tanggalLahir || !alamat || !asalSekolah || !noHp || !pendapatanOrtu) {
        alertBox.innerHTML = `<div class="alert error">Semua field wajib diisi kecuali email.</div>`;
    ...
    const data = {
        ...
        no_hp: noHp,
        email: email,
        pendapatan_ortu: Number(pendapatanOrtu),
        status: "menunggu",
        tanggal_daftar: new Date().toISOString()
    };

    try {
        console.log("Mulai kirim ke Firebase...", data);

        await Promise.race([
            addDoc(collection(db, "pendaftaran"), data),
            new Promise((_, reject) =>
                setTimeout(() => reject(new Error("Request timeout: Firebase terlalu lama merespons.")), 15000)
            )
        ]);
...
Saya atas nama *${namaLengkap}* telah melakukan pendaftaran PPDB.
NISN: ${nisn}
...
Mohon konfirmasi pendaftaran saya. Terima kasih.`;
        ...
        alertBox.innerHTML = `<div class="alert success">Pendaftaran berhasil dikirim silahkan di tunggu.</div>`;
        modal.classList.add("active");
        form.reset();
    } catch (err) {
        console.error("Firebase error:", err);
        alertBox.innerHTML = `<div class="alert error">Gagal menyimpan data: ${err.message}</div>`;
    } finally {
        submitBtn.disabled = false;
        submitBtn.innerHTML = originalText;
    }
